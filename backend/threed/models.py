...
def add_3d_to_product(sender, instance, **kwargs):
    """
    when create product we set threed for product
    """
    thd_obj = ThreeD.objects.filter(product=instance)
    if thd_obj:
        pass
    else:
        ThreeD.objects.create(product=instance)
